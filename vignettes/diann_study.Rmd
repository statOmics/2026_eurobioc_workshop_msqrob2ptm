...
```{r heatmaps_ptm_usage, eval=FALSE}
lapply(colnames(L),
                   function(contrast, se, alpha)
                   {
                     sig <- rowData(se)[[contrast]] |> 
...
Show significance all phospho's on protein `r sub("_.*","", target_feature)` for
contrast `strainProAla - strainCtrl`

```{r}
prot <- sub("_.*","", target_feature)
inferencesPtmUsage |> 
  filter(contrast == "strainProAla - strainCtrl" & grepl(prot, feature))
```
