...
- Typically we do not specify the path to the cache file in local sessions. 
- If you run the vignette(or code-chunk below) on your local machine in your 
  local R session (without using the docker) comment first line and 
  uncomment second line. 

```{r data_path}
bfc <- BiocFileCache("/opt/fileCache") # for usage with docker
...
We only do this for the non-enriched assay (whole proteome).
...
We first performa the QC for the Phospho-enriched runs then for the non-enriched
runs. 
